...
random_state = 68
run_name = "run-gpu1"
logdir = 'logs/tensorboard/99-fft/'
...
def train_test_model(logdir, hparams):
    ...
    history = model.fit(x_train, y_train, validation_data=(x_test, y_test), batch_size=16, epochs=250, callbacks=cb, verbose=0)
    ...
    for units in ['[128, 64]', '[128]', '[64, 8]', '[32]']:
        for dropout in [0.3, 0.5]:
            for lr in [0.0001, 0.00001, 0.01]:
                for kernel_1 in ['[1, 5]', '[1, 3]', '[1, 10]', '[2, 5]']:
                    for filter_1 in [8, 32, 128]:
                        for filter_kernel_2 in ['[0, 0]', '[8, 3]', '[32, 5]', '[128, 5]']:
                            hparams = {
                                'units': units,
                                'dropout': dropout,
                                'lr': lr,
