...
HP_CNN_FILTER_3 = hp.HParam('filter_3', hp.Discrete([8, 32, 64]))
HP_CNN_KERNEL_1 = hp.HParam('kernel_1', hp.Discrete([5, 10]))
HP_CNN_KERNEL_2 = hp.HParam('kernel_2', hp.Discrete([5, 10]))
HP_CNN_KERNEL_3 = hp.HParam('kernel_3', hp.Discrete([5, 10]))

with tf.summary.create_file_writer('logs/80-small-v1').as_default():
    hp.hparams_config(
        hparams=[HP_NUM_UNITS, HP_LEARNING_RATE, HP_CNN_KERNEL_1, HP_CNN_KERNEL_2, HP_CNN_KERNEL_3, HP_CNN_FILTER_1, HP_CNN_FILTER_2, HP_CNN_FILTER_3],
...
for idx, row in tqdm(df_params.iterrows(), total=df_params.shape[0]):
    hparams = {
        ...
        HP_CNN_KERNEL_3: row['kernel_3'],
        HP_CNN_FILTER_1: row['filter_1'],
        HP_CNN_FILTER_2: row['filter_2'],
        HP_CNN_FILTER_3: row['filter_3']
    }
    run_name = "run2-%d" % session_num
    train_test_model('logs/tensorboard/80-small-v1/' + run_name, hparams)
    session_num += 1
