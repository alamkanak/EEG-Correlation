...
def read_wavelets(epoch, region):
    time = epoch['time'].values
    channel = filter_electrodes(epoch, region).drop('time', axis=1).mean(axis=1).values
    wavelet = get_wavelet(channel, time, [260, 260])
    return wavelet

def save_wavelets_of_epoch(save_destination, epoch_num, epoch, path_segments):
    if not os.path.exists(save_destination):
        os.makedirs(save_destination)
    for channel in ['Fp1', 'Fpz', 'Fp2', 'F7', 'F3', 'Fz', 'F4', 'F8', 'FC5', 'FC1', 'FC2', 'FC6', 'M1', 'T7', 'C3', 'Cz', 'C4', 'T8', 'M2', 'CP5', 'CP1', 'CP2', 'CP6', 'P7', 'P3', 'Pz', 'P4', 'P8', 'POz', 'O1', 'O2', 'EOG', 'AF7', 'AF3', 'AF4', 'AF8', 'F5', 'F1', 'F2', 'F6', 'FC3', 'FCz', 'FC4', 'C5', 'C1', 'C2', 'C6', 'CP3', 'CP4', 'P5', 'P1', 'P2', 'P6', 'PO5', 'PO3', 'PO4', 'PO6', 'FT7', 'FT8', 'TP7', 'TP8', 'PO7', 'PO8', 'Oz']:
        wt = read_wavelets(epoch, channel).values
        try:
            wt_3d = np.dstack((wt_3d, wt))
        except NameError:
            wt_3d = wt
    filename = save_destination + '/' + path_segments[2] + '-' + path_segments[3] + '-' + path_segments[5] + '-' + str(epoch_num) + '.pickle'
...
# save_wavelets_of_epoch(save_destination, 0, epochs[0], segments)

#%%
num_cores = multiprocessing.cpu_count()
save_destination = 'wavelets-100'
...
#%%
for wavelet_path in glob.glob(save_destination + '/*.pickle')[0:2]:
    print(wavelet_path)
    with open(wavelet_path, 'rb') as f:
        wavelet = pickle.load(f)
        print(wavelet.shape)
        fig = plt.figure(figsize=(6,4))
        sns.heatmap(wavelet[:, :, 0])
        fig = plt.figure(figsize=(6,4))
        sns.heatmap(wavelet[:, :, 10])
        fig = plt.figure(figsize=(6,4))
        sns.heatmap(wavelet[:, :, 20])
