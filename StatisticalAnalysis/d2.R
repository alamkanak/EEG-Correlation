...
df2 %>% group_by(trial_abs) %>% identify_outliers(Power)
df2 %>% group_by(trial_abs) %>% shapiro_test(Power)
res.aov <- anova_test(data=df2, dv=Power, wid=sub, within=ArtifactRemoved)
