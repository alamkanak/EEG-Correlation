...
with tf.summary.create_file_writer('logs/92-c3-v1').as_default():
    hp.hparams_config(
...
def train_test_model(logdir, hparams):
    ...
    classifier.fit(x_train, y_train, validation_data=(x_test, y_test), batch_size=64, epochs=500, callbacks=cb, verbose=0)
...
for idx, row in tqdm(df_params.iterrows(), total=df_params.shape[0]):
    ...
    train_test_model('logs/tensorboard/92-c3-v1/' + run_name, hparams)
    session_num += 1
