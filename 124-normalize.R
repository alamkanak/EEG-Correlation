...
df = read_excel("137-all-v3.xlsx'")
df = subset(df, df$rejected == FALSE)
...
df$C3_gamma_phase_bin <- cut(df$C3_gamma_phase, breaks=c(0,180,360), labels=c('peak', 'trough'))
df$C4_gamma_phase_bin <- cut(df$C4_gamma_phase, breaks=c(0,180,360), labels=c('peak', 'trough'))
# df$LTM1_gamma_phase_bin <- cut(df$LTM1_gamma_phase, breaks=c(0,180,360), labels=c('peak', 'trough'))
df$C3_gamma_phase_bin <- cut(df$C3_gamma_phase, breaks=c(0,180,360), labels=c('peak', 'trough'))
df$C4_gamma_phase_bin <- cut(df$C4_gamma_phase, breaks=c(0,180,360), labels=c('peak', 'trough'))
# df$LTM1_gamma_phase_bin <- cut(df$LTM1_gamma_phase, breaks=c(0,180,360), labels=c('peak', 'trough'))
df$C3_high_gamma_phase_bin <- cut(df$C3_high_gamma_phase, breaks=c(0,180,360), labels=c('peak', 'trough'))
df$C4_high_gamma_phase_bin <- cut(df$C4_high_gamma_phase, breaks=c(0,180,360), labels=c('peak', 'trough'))
# df$LTM1_high_gamma_phase_bin <- cut(df$LTM1_high_gamma_phase, breaks=c(0,180,360), labels=c('peak', 'trough'))
df$C3_low_gamma_phase_bin <- cut(df$C3_low_gamma_phase, breaks=c(0,180,360), labels=c('peak', 'trough'))
df$C4_low_gamma_phase_bin <- cut(df$C4_low_gamma_phase, breaks=c(0,180,360), labels=c('peak', 'trough'))
# df$LTM1_low_gamma_phase_bin <- cut(df$LTM1_low_gamma_phase, breaks=c(0,180,360), labels=c('peak', 'trough'))
df$C3_theta_phase_bin <- cut(df$C3_theta_phase, breaks=c(0,180,360), labels=c('peak', 'trough'))
df$C4_theta_phase_bin <- cut(df$C4_theta_phase, breaks=c(0,180,360), labels=c('peak', 'trough'))
...
df$C3_gamma_phase_bin <- factor(df$C3_gamma_phase_bin)
df$C4_gamma_phase_bin <- factor(df$C4_gamma_phase_bin)
# df$LTM1_gamma_phase_bin = factor(df$LTM1_gamma_phase_bin)
df$C3_high_gamma_phase_bin <- factor(df$C3_high_gamma_phase_bin)
df$C4_high_gamma_phase_bin <- factor(df$C4_high_gamma_phase_bin)
# df$LTM1_high_gamma_phase_bin = factor(df$LTM1_high_gamma_phase_bin)
df$C3_low_gamma_phase_bin <- factor(df$C3_low_gamma_phase_bin)
df$C4_low_gramma_phase_bin <- factor(df$C4_low_gamma_phase_bin)
# df$LTM1_low_gamma_phase_bin = factor(df$LTM1_low_gamma_phase_bin)
df$C3_theta_phase_bin <- factor(df$C3_theta_phase_bin)
df$C4_theta_phase_bin <- factor(df$C4_theta_phase_bin)
# df$LTM1_theta_phase_bin = factor(df$LTM1_theta_phase_bin)

df$C3_mu_power <- log(df$C3_mu_power)
df$C3_beta_power <- log(df$C3_beta_power)
df$C3_gamma_power <- log(df$C3_gamma_power)
df$C3_low_gamma_power <- log(df$C3_low_gamma_power)
df$C3_high_gamma_power <- log(df$C3_high_gamma_power)
df$C3_theta_power <- log(df$C3_theta_power)
df$C4_mu_power <- log(df$C4_mu_power)
df$C4_beta_power <- log(df$C4_beta_power)
df$C4_gamma_power <- log(df$C4_gamma_power)
df$C4_low_gamma_power <- log(df$C4_low_gamma_power)
df$C4_high_gamma_power <- log(df$C4_high_gamma_power)
df$C4_theta_power <- log(df$C4_theta_power)
...
df$C3_gamma_power_abs <- abs(df$C3_gamma_power)
df$C3_theta_power_abs <- abs(df$C3_theta_power)
df$C4_mu_power_abs <- abs(df$C4_mu_power)
df$C4_beta_power_abs <- abs(df$C4_beta_power)
df$C4_gamma_power_abs <- abs(df$C4_gamma_power)
df$C4_theta_power_abs <- abs(df$C4_theta_power)
...
print_mm <- function(channel, band, response, removeOutlier=TRUE, abs='') {
  power = paste(c(channel, band, 'power'), collapse='_')
  power = paste(c(power, abs), collapse='')
  phase = paste(c(channel, band, 'phase_bin'), collapse='_')
...
plot_interaction <- function(channel, band, response, removeOutlier=True, abs='') {
  power = paste(c(channel, band, 'power'), collapse='_')
  power = paste(c(power, abs), collapse='')
...
plot_and_mm <- function(channel, band, response, abs=FALSE, removeOutlier=FALSE) {
  abs <- if(abs == TRUE) '_abs' else ''
  plot_interaction(channel, band, response, removeOutlier, abs=abs)
  print_mm(channel, band, response, removeOutlier, abs=abs)
}
...
df$C3_b2m = df$C3_mu_power / df$C3_beta_power
df$C3_b2m = log(abs(df$C3_b2m/mean(df$C3_b2m)))
df$C3_b2m_power = df$C3_b2m
df$C3_b2m_power_abs = abs(df$C3_b2m_power)

df$C3_b2g_power = df$C3_beta_power / df$C3_gamma_power
df$C3_b2g_power = log(abs(df$C3_b2g_power/mean(df$C3_b2g_power)))

df$C3_m2g_power = df$C3_mu_power / df$C3_gamma_power
df$C3_m2g_power = log(abs(df$C3_m2g_power/mean(df$C3_m2g_power)))
df$C3_m2g_power_abs = abs(df$C3_m2g_power)
...
y = 'mep_size_log'
m = plot_and_mm('C3', 'theta', y, abs=FALSE, removeOutlier=TRUE)
summary(m)
qqp(resid(m), 'norm')
plot(residuals(m))
qqmath(m)
...
contrasts(df$C3_theta_phase_bin) <- "contr.sum"
...
m = glmer(mep_duration_dis ~ (C3_beta_power * C3_beta_phase_bin) + (1|sub), data=df, family='poisson', nAGQ=0)
summary(m)
plot(df$C3_beta_power, df$mep_duration_dis)
...
m = glmer(mep_latency_dis ~ (C3_gamma_power * C3_gamma_phase_bin) + (1|sub), data=df, family='Gamma', nAGQ=0)
summary(m)
plot(df$C3_gamma_power, df$mep_latency_dis)
...
# --------------------
hist(df$mep_size_log, breaks=40)
m = glmer(mep_size_log_dis ~ (C3_beta_power * C3_beta_phase_bin) + (1|sub), data=df, family='Gamma', nAGQ=0)
summary(m)
plot(df$C3_beta_power, df$mep_size_log_dis)
...
# GLMM
df$mep_latency_dis = as.integer(normalize(df$mep_latency, method='range', range=c(1, 100)))
m = glmer(mep_latency_dis ~ (C3_mu_power * C3_mu_phase_bin) + (1|sub), data=df, family=poisson, nAGQ=0)
summary(m)
poisson <- fitdistr(resid(m), "Poisson")
qqp(resid(m), 'pois', lambda=poisson$estimate)
plot(df$C3_mu_power, df$mep_size)
